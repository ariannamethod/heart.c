                        ...
                        for (int h = 0; h < n_heads; h++) {
                            ...
                            for (int i = 0; i < half; i++) {
                                ...
                                if (split_half) {
                                     ...
                                     * Backward: dx0 = c*dn0 + s*dn1; dx1 = -s*dn0 + c*dn1 */
                                    gx[o0] = dx0 * cos_a + dx1 * sin_a;
                                    gx[o1] = -dx0 * sin_a + dx1 * cos_a;
                                }
                            }
...
int nt_rope_split_half_freq(int x_idx, int T, int head_dim, float freq_base) {
     ...
     * Used by nanochat / Janus v4 (infer_v4.c:35-49). CPU-only forward.
     * Dispatches via NT_OP_ROPE with aux4=1.0 — backward branches on aux4. */
    if (x_idx < 0 || T <= 0 || head_dim <= 0) return -1;
    if (freq_base <= 0.0f) freq_base = 10000.0f;
    nt_tape_entry* px = &g_tape.entries[x_idx];
    int total = px->output->len;
    ...
    for (int t = 0; t < T; t++) {
        for (int h = 0; h < n_heads; h++) {
            ...
            for (int i = 0; i < half; i++) {
                ...
                /* Match canonical Janus v4 rope_pos sign convention
                 * (infer_v4.c:42-49):
                 *   q[i]        = q0 * cv + q1 * sv;
                 *   q[i + half] = -q0 * sv + q1 * cv;
                 * Inverse-rotation form vs notorch's even/odd default. */
                out->data[base + i]        =  x0 * cos_a + x1 * sin_a;
                out->data[base + half + i] = -x0 * sin_a + x1 * cos_a;
            }
        }
    }
    /* CPU is source of truth; ensure next GPU op re-uploads. */
#ifdef USE_CUDA
